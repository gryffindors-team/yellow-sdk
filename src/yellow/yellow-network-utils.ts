...
export const sessionKeyAtom = atomWithStorage<SessionKey | null>(
  "yellow_network_session_key",
  null,
  undefined,
  { getOnInit: true }
);

export const jwtTokenAtom = atomWithStorage<string | null>(
  "yellow_network_jwt_token",
  null,
  undefined,
  { getOnInit: true }
);
...
export const eip712SignatureAtom = atomWithStorage<{
  signature: `0x${string}`;
  timestamp: number;
  ...
  "yellow_network_eip712_signature",
  null,
  undefined,
  { getOnInit: true }
);
...
export const validateSessionKey = (
  ...
  if (!sessionKey || typeof sessionKey !== "object") return false;
 ...
 */
export const validJwtTokenAtom = atom<string | null>((get) => {
 ...
 */
export const isEIP712SignatureValid = (
  signature: {
    signature: `0x${string}`;
    timestamp: number;
    sessionKeyAddress: Address;
  ...
  currentSessionKeyAddress: Address,
  currentAccount: Address,
  currentExpireTimestamp: string
): boolean => {
  if (!signature) return false;

  // Check if signature matches current session
  if (
    signature.sessionKeyAddress.toLowerCase() !== currentSessionKeyAddress.toLowerCase() ||
 ...
 */
export const getFutureTimestamp = (secondsFromNow: number): string => {
